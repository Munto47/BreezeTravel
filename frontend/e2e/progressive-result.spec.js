...
test('progress cards stay read-only until stop promotes the last snapshot to an editable result', async ({ page }) => {
  ...
  await expect(page.getByTestId('generation-stages').locator('[data-state=done]')).toHaveCount(2)
  await expect(page.locator('.fluid-paper')).toHaveCount(3)
  await expect(page.locator('.fluid-scan-beam')).toHaveCSS('animation-name','scan-paper')
  await page.emulateMedia({reducedMotion:'reduce'})
  await expect(page.locator('.fluid-paper').first()).toHaveCSS('animation-name','none')
  ...
  await page.getByRole('button', { name: '停止整理' }).click()
