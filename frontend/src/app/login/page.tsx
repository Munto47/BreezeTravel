...
export default function LoginPage() {
  ...
  const handleSendCode = async () => {
    ...
    setLoading(true)
    try {
      const res = await boundedFetch(`${API_BASE}/api/auth/send-code`, {
        method: 'POST',
  ...
  const verifyCode = async (fullCode: string) => {
    ...
    setLoading(true)
    try {
      const res = await boundedFetch(`${API_BASE}/api/auth/verify`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleTestLogin = async () => {
    ...
    setLoading(true)
    try {
      const res = await boundedFetch(`${API_BASE}/api/auth/test-login`, { method: 'POST' })
      const data = await res.json()
  ...
  const handleEmailSubmit = async () => {
    ...
    try {
      ...
      const res = await boundedFetch(`${API_BASE}${endpoint}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
