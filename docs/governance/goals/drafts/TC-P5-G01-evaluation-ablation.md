# DRAFT GOAL：P5-G01 统一评测、隔离盲测与消融决策
...
- Phase：`P5`
- Status：`DRAFT`
- Planned branch：`codex/trip-check-p5-evaluation-ablation`
- Proposed baseline commit：`c8a5a0f6df3b4cef0d707742fa616eb5652ca6cc`
...
本文件只制定 P5 目标与执行计划。它不替换 `CURRENT_GOAL.md`，也不授权开始实现、修改 frozen blind/oracle、进入 P6、公网或真人阶段。
...
### Milestone 0：批准、集成基线与合同冻结（第 1～2 天）

- 经现场批准后，才把本 Goal 改为 `APPROVED` 并替换 `CURRENT_GOAL.md`；
...
## Pre-approved actions after approval
...
本节只有在 Goal 经现场批准并进入 `APPROVED/IN_PROGRESS` 后生效；当前 DRAFT 不授予实现权限。
