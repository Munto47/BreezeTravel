...
- Canonical integration subject：`origin/develop@c5a6f847f6c904dfaa81a397c2af878b18f089a6`，PR #16合并与远端readback `PASS`
...
|---|---|---|---|
| `WP-G04-INTEGRATOR` | `codex/g04-screenshot-integration` / `...-g04-integration` | governance、034、共享API/持久化/worker、OpenAPI/client、首页装配、CI、最终E2E | `MERGED`（delivery `8542db70` / integration `c5a6f847`） |
...
- 2026-08-31 Owner批准方案A作为临时`HISTORICAL_BINDING_INVALID_FROZEN`兼容例外：完整运行非P5套件，只允许两条已登记Trip NLU节点在`call`阶段以固定`manifest evaluator/schema code binding mismatch`指纹失败；预期失败缺失、增加、重复、换阶段、换指纹，或collection/internal/interrupt/退出码异常均失败关闭。例外同时绑定base/candidate manifest、validator、scorer、gate、原测试、generator和运行时schema的当前hash，不授权修改任何冻结资产或阈值，并须在G07 exact-binding验收前移除。
- Owner随后明确两个Git外许可截图源目录无需删除；它们保持为Owner控制的本地评测来源副本，不属于上传API运行时临时文件、数据库或Git证据。此决定不改变所有运行时终态原图清理及receipt 100%的产品Gate。
...
| 2026-08-31 | G04截图入口经耐久交付回执和受保护PR正式进入`develop`；用户可从1至6张截图获得与文本入口一致的逐日卡片，运行时原图在终态确认清理 | delivery `8542db708f8a1d450750d9050424088ce7756bf1`；integration `c5a6f847f6c904dfaa81a397c2af878b18f089a6` | GitHub Actions `33357640834`在delivery精确tip上六项required job及`core-mainline`全部PASS；PR #16 `MERGED`；`origin/develop@c5a6f847f6c904dfaa81a397c2af878b18f089a6` fetch/ls-remote一致 | `REMOTE_AUTOMATED / PRODUCT_DELIVERY_PASS / REAL_PADDLE_LICENSED_SCREENSHOT_PARITY / REMOTE_INTEGRATION_PASS` | `Product progress=API+RUNTIME+UI / COMPLETE` | `Governance ratio=atomic transition only after integration` | 原子激活G05三城有来源知识层 | 方案A兼容例外仍须在G07 exact-binding前移除；Qwen-VL、H1、公网、生产、商业、发布和`main`保持`NOT_RUN / NOT_AUTHORIZED`；两个Git外来源目录按Owner决定保留 | 归档本完整合同，把CURRENT_GOAL、binding与工作包原子切换到G05，不实现G05产品代码 |

## Auto-advance

- Required gate：`Screenshot Parity Gate + PRODUCT_DELIVERY_PASS`；Next Goal：`TC-VNEXT-G05-CITY-KNOWLEDGE`。
- subject `8542db708f8a1d450750d9050424088ce7756bf1`已push/readback，耐久PASS与clean tree满足；PR #16已并入`origin/develop@c5a6f847f6c904dfaa81a397c2af878b18f089a6`，本归档与G05激活在独立治理过渡commit中原子完成；不创建外部authority generation，不自动进入H1/公网/生产。

## Completion record

- Status：`COMPLETED`；Subject commits / Remote branch：产品候选`525af072c47a3f318d88c722bf8067d6ff30907c`，方案A`79e99555baa97c2d3d75b77c9d50ed158df36327`，delivery `8542db708f8a1d450750d9050424088ce7756bf1` / `origin/codex/g04-screenshot-integration`；canonical integration `origin/develop@c5a6f847f6c904dfaa81a397c2af878b18f089a6`；
- Verification / Evidence / Gate result / structurally_valid：`REMOTE_REQUIRED_CI_PASS / REAL_PADDLE_LICENSED_SCREENSHOT_PARITY + REAL_POSTGRESQL + BROWSER_E2E_FIXTURE + EXACT_HISTORICAL_RENDERER_ABI + REMOTE_INTEGRATION_PASS / PRODUCT_DELIVERY_PASS / true`（`structurally_valid=true`）；GitHub Actions `33357640834 PASS`；
- 当前结构有效或G04自动验证通过，不得因此宣称 `V1_CANDIDATE_READY`；该发布门仍不在本Goal授权和证据范围内。
- H1 / production / commercial：`NOT_RUN / NOT_RUN / NOT_RUN`；
- User-visible result / Remaining risks / Goal archived / Next activated：`截图入口与文本入口的一致卡片体验已通过Screenshot Parity Gate、生成PRODUCT_DELIVERY_PASS并合入develop，两个Git外许可截图源目录按Owner决定保留 / 临时精确历史兼容例外须在G07 exact-binding验收前移除；Qwen-VL、H1、公网、生产、商业、发布和main无证据或未授权 / true / TC-VNEXT-G05-CITY-KNOWLEDGE APPROVED`；
