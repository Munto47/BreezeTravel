...
每个Goal通过`current_work_packages.json`拆分为一个集成者和工作包；所有同时可写包（包括写入中的集成者）合计最多三个且路径互不重叠。仅可为下一Goal提前准备最多两个包，状态为`PREPARED_NOT_INTEGRATED`；并行实现、串行合并。完整机器规则见Program 3.3。
