...
每包填写`package_id / goal_id / baseline_commit / branch / role / dependencies / owned_paths / forbidden_paths / acceptance / merge_order / status`。固定一个非终态集成者；所有同时可写包（含写入中的集成者）合计最多三个，`READY_TO_MERGE`冻结后不得继续写；最多提前准备下一Goal两个`PREPARED_NOT_INTEGRATED`包。写明领域模型→持久化/API→前端→E2E的串行合并点。
