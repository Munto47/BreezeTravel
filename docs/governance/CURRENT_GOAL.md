...
| 2026-09-03 | `9a519ca`的自动、真实Provider、PostgreSQL、浏览器和50链性能均通过，但fresh独立复审发现目的地绑定与来源删除两个P1；候选已拒绝且sealed未启动，两个P1已完成最小运行时修复 | rejected subject `9a519ca3c0eacf359302158b6feaf0a1df480290`；tree `5a81e31f8c8234d9b2661047d2bf20e1eb243c65`；repair subject为本checkpoint | 旧候选产品复审PASS、语义复审FAIL、可靠性复审FAIL；语义复现为72/72模型目的地错误并进入公开目的地与Provider城市通道；可靠性复现为删除后无原子地点PLANNED活动仍保留mention_text。修复后冻结72条城市语境提取`72/72`、语义/Qwen `44 passed`、PostgreSQL显式删除与TTL `2 passed`、扩大语义/隐私/地图/数据库 `150 passed`、Ruff、diff check与core-mainline PASS | `MULTI_AGENT_SIMULATED_REVIEW_FAIL / LOCAL_RUNTIME_REPAIR / CONTROLLED_POSTGRESQL_PASS` | `Product progress=RUNTIME+SEMANTIC+PRIVACY / CANDIDATE_REPAIR` | `Governance ratio=失败checkpoint与精确修复范围；未改Gate、阈值、模型、数据、oracle或blind` | 提交、push并远端回读repair subject；随后刷新exact binding并从全新Git外证据根重跑全部正式组件和fresh panel，panel通过后才启动sealed | `9a519ca`全部PASS组件只作失败诊断；OCR冻结许可截图原始字节仍缺失并将继续fail-closed；Provider崩溃后缺少STARTED持久回执的P2不影响保守接管行为，但须由fresh reliability复审和最终裁决判定当前G07归属 | 先完成候选绑定的两阶段提交与clean checkout复核，再执行G0～G5和独立复审；不得复用旧证据或提前运行blind |
