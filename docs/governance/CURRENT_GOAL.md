# IN_PROGRESS GOAL：V0.5 三城有来源知识层
...
|---|---|---|---|
| `WP-G05-INTEGRATOR` | `codex/g05-city-knowledge` / `...-g05-integration` | 来源准入、032、共享schema/repository/API、OpenAPI/client、建议UI、消融、CI与最终E2E | `IN_PROGRESS` |

当前registry为`INTEGRATOR_ONLY`，没有激活贡献writer或独立任务；同一集成者按来源领域/许可 → 032与内部API → 建议投影/UI/消融/E2E串行推进，避免共享合同并行写入。
...
|---|---|---|---|---|---|---|---|---|---|
| 2026-08-31 | 北京、上海、杭州地点卡已可动态装配最多3条有来源建议；未来版本不提前遮蔽当前建议，撤回、过期、冲突或检索失败不会改变行程、地图、POI、路线或Finding；删除行程会同步清除使用回执 | 本产品checkpoint（提交后push/readback） | 来源Gate与消融 `PASS`：18地点逐一处置、4个准入来源、1个`NOT_READY`、4种有claim、典型时长明确缺口，precision `1.0`、unsupported `0`、actionability提升 `16.67pp`、P95回退低于`20%`；定向 `57 passed`；PostgreSQL 032 fresh/after-034、不同bundle并发导入、幂等重放、未来版本、版本升级、并发撤回、claim/source撤回、使用回执与隐私删除 `3 passed`；治理 `46 passed`；OpenAPI/client、frontend build、Playwright `3 passed`、Ruff和diff check均通过 | `OFFICIAL_PUBLIC_HTTPS_FACTS_ONLY / LOCAL_POSTGRESQL / AUTOMATED_FIXTURE_BROWSER` | `Product progress=IMPLEMENTED / DELIVERY_VERIFY_IN_PROGRESS` | `Governance ratio=bounded to G05 contract` | 提交push并取得首轮exact-tip CI；生成耐久交付回执后在回执tip复跑CI并经PR合入 | 冻结集中典型游览时长没有合规精确来源，保持`EXPLICIT_GAP`；G04两个精确历史失败例外原样保留至G07；本地浏览器为自动fixture而非公网或真人证据 | 创建checkpoint commit并运行core-mainline范围校验、push/readback与PR门禁 |
...
- Status：`IN_PROGRESS`；Subject commits / Remote branch：本产品checkpoint（提交后push/readback） / `origin/codex/g05-city-knowledge`尚未push；
