...
7. 并行开发只认`current_work_packages.json`：一个非终态集成者、所有同时可写包合计最多三个、同一exact baseline、owned paths不重叠、普通包不改治理binding/migration/OpenAPI生成物/锁文件；`READY_TO_MERGE`冻结后不可再写，不一致的worktree只读。
