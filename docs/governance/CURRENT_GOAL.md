# CURRENT GOAL：P6-G01 候选证据、受控公网与 Candidate Gate
...
- Commits：`NOT_COMPLETED`；
- Remote branch / upstream：待首次 P6 checkpoint push；
- Verification results：G0～G6=`NOT_RUN`；
- Evidence paths：待 candidate commit 冻结后生成；
- Gate result：`Candidate Gate=NOT_RUN`；
- Remaining red lights：real OCR、PostgreSQL、snapshot、18 live receipts、本地/公网 E2E、性能、manifest、视频全部未运行；
