...
已修复原文限定名、并列地点、二选一、整日A/B、跨天重复、同日备选再访和错误城市查询；封住“前门”误确认为博物馆，以及短入住引用清掉具名酒店、跨行只选多补地点的问题。895项解析与边界测试、相关141项数据库/API、12项浏览器检查已在近期代码通过；具体对应版本与失败记录见[七份实际攻略复现记录](OWNER_GUIDE_REPRODUCTION_20260906.md)。

完整页面after-r9为6/7；前6份71个有名主站、31个备选的名称/数量/顺序/日归属/角色符合，46个主站核对到真实地点，25个保守待确认。已修正未确认街区类别及提前购票导致日期检查退出的问题；比较后将同一模型采样温度设为0，仍为两次调用共享60秒，不保证服务逐字确定。最新after-r10仍为6/7：第6、7份已返回，第4份一个原文引用两次无法定位。固定24份原创回归r8全部通过，24调用0修复；不能拼接不同轮成功宣称7份通过。正在缩小第4份引用问题，下一自主动作是修复后重新完成同版本七份及相关回归，无需所有者排查。下面为上一轮记录，不能替代当前7份验收。
