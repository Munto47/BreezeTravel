...
|---|---|---|---|---|---|
| 文本语义质量上限 | Qwen Max exact binding | G01预批准实验，配置待readback | 开发benchmark候选 | 只存receipt/hash/指标，不存密钥与完整原文 | 账号、区域、endpoint、exact ID、隐私与模型Gate |
| 文本语义生产候选 | Qwen Plus exact binding | G01预批准实验，配置待readback | Validation达标后可成为唯一候选 | 同上 | 与Max同dev/validation比较，不能用于blind选模 |
| 文本低延迟候选 | Qwen Flash exact binding | G01预批准实验，配置待readback | 否 | 同上 | dev/validation的schema、质量和P95均达标 |
...
| POI搜索 | 高德Web Service | `PROPOSED`，G01许可readback后开发 | G01候选 | 许可未确认前`BLOCKED_PERSISTENCE`；只允许批准的最小字段 | 数据留存/展示书面澄清 |
| 路线 | 高德步行/公交 | `PROPOSED`，G01许可readback后开发 | G01后台/G02展示候选 | 许可未确认前`BLOCKED_PERSISTENCE`；geometry默认短期缓存 | 商业展示、缓存和域名许可 |
...
binding还必须记录非思考模式、温度、prompt/schema/config hash、pricing version与币种。不存在exact ID或区域readback时不得启动候选运行。
...
5. P95至少改善20%才因性能替换。
6. 只选一个候选并冻结全部binding后运行sealed blind一次；blind标签由独立custodian保管，模型和开发代理不可读。
7. blind失败进入新dev/regression故障族，不能回看标签选另一个模型；只有输入分布/schema变化并经独立批准才创建新blind版本。
8. 失败模型保持实验或Baseline，不通过文案掩盖。

## 4. 高德边界

G01/G02只在现有开发授权和无增量费用范围调用。

以下字段只是拟议最小集，只有条款/许可readback明确允许后才能持久化：
...
路线geometry和完整响应默认只进入受限短期缓存。生产、公开演示或商业化前必须核对高德条款并取得对缓存、拆分展示、持久化和商业使用的明确许可。未解决时状态为`BLOCKED_PRODUCTION`，不是开发代码失败。
