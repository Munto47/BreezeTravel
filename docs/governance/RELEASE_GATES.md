...
每个Goal晋级前，`current_work_packages.json`必须证明：恰好一个非终态集成者、所有同时可写包合计最多三个、`READY_TO_MERGE`已冻结、最多两个下一Goal准备包、同一exact baseline、路径所有权无重叠、普通包未触碰受保护文件、所有包已按merge order串行整合。指导或binding不一致、跨两级开发、`PREPARED_NOT_INTEGRATED`提前合并，任一项均阻断Gate。
