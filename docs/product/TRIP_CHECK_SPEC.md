...
当前产品只接受文本，不接收截图，不建设、调用或验证OCR。既有截图上传、PaddleOCR、Qwen-VL消融、OCR文本、阅读顺序和bbox链均为冻结历史资产：公开运行时默认关闭，普通用户界面和公共API不得出现，当前候选Gate、性能指标和完成声明不得引用。未来图片理解需要新的项目所有者批准和独立产品/隐私合同，不能作为本合同的隐含能力。
...
用户记忆显式开启，只保存可查看、可修改、可删除的结构化偏好。原始攻略和聊天默认不进入长期记忆；训练或评测需要单独同意。
