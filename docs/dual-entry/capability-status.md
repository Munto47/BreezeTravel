...
| 版本化 TripBrief 与确认档案复用 | planned | 需要从 migration 022 开始追加；`NO_PREFERENCE` 和本次优先合同未实现 |
...
| TripCheckRun 阶段持久化、SSE 与重启恢复 | planned | 现有 resume/lease 资产可复用，但没有 V1 阶段/config hash 合同 |
| 360 条 import-only v2 数据集 | planned | 目标为 18 pilot / 180 dev / 72 regression / 90 frozen blind，三城各 120 |
