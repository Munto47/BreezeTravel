"""Anonymous, three-round automated Judge boundary for P5 v4.

The module deliberately uses a delayed blind-run validator instead of importing
``contracts_v4``.  This keeps the Judge boundary mergeable while the v4 runner
...
DIMENSIONS_V4 = ("clarity", "actionability", "evidence_boundary_expression")
ROUND_COUNT_V4 = 3
BLIND_CASE_COUNT_V4 = 90
BLIND_TERMINAL_COUNT_V4 = 270
JUDGE_AGREEMENT_THRESHOLD_V4 = 0.85


class P5JudgeErrorV4(RuntimeError):
...
BlindRunValidatorV4 = Callable[..., object]
...
def _sha256(path: Path) -> str:
    try:
        ...
        raise P5JudgeErrorV4("JUDGE_ARTIFACT_UNREADABLE") from exc
...
def _load_json(path: Path, reason: str) -> dict[str, Any]:
    try:
        ...
        raise P5JudgeErrorV4(reason) from exc
    if not isinstance(value, dict):
        raise P5JudgeErrorV4(reason)
    return value
...
def _require_external_distinct_dirs(repo_root: Path, paths: Sequence[Path]) -> list[Path]:
    if len(paths) != ROUND_COUNT_V4 + 1 or any(not path.is_absolute() for path in paths):
        raise P5JudgeErrorV4("JUDGE_CUSTODY_PATH_INVALID")
    ...
    if len({str(path.resolve()) for path in resolved}) != len(resolved):
        raise P5JudgeErrorV4("JUDGE_CUSTODY_PATH_NOT_DISTINCT")
...
def _default_blind_run_validator_v4(
    *, run_dir: Path, repo_root: Path, require_formal: bool
    ...
    candidates = (
        ...
        (
            "evals.trip_check_v1.p5.final_blind_scorer_v4",
            "validate_blind_run_group_v4",
        ),
        ("evals.trip_check_v1.p5.scorer_v4", "validate_blind_run_group_v4"),
    )
    ...
    raise P5JudgeErrorV4("V4_BLIND_RUN_VALIDATOR_UNAVAILABLE")


def _normalize_validated_run_v4(result: object) -> tuple[
    dict[str, Any], list[object], list[object], dict[str, Mapping[str, Any]]
]:
    if not isinstance(result, tuple) or len(result) != 4:
        raise P5JudgeErrorV4("V4_BLIND_RUN_VALIDATOR_PROTOCOL_INVALID")
    ...
    if len(materializations) != len(raw_cases):
        raise P5JudgeErrorV4("JUDGE_MATERIALIZATION_SET_INVALID")
    return manifest, raw_cases, raw_outputs, materializations


def _validate_blind_run_shape_v4(
    ...
    expected_keys = {
        (case_id, variant_id) for case_id in case_ids for variant_id in VARIANT_IDS_V4
    }
    required_hashes = (
        "manifest_hash",
    ...
    if (
        len(cases) != BLIND_CASE_COUNT_V4
        or len(outputs) != BLIND_TERMINAL_COUNT_V4
        ...
        or manifest.get("case_count") != BLIND_CASE_COUNT_V4
        or manifest.get("terminal_count") != BLIND_TERMINAL_COUNT_V4
        or manifest.get("replay_executed") is not True
        or manifest.get("replay_readback_count") != BLIND_TERMINAL_COUNT_V4
        ...
        raise P5JudgeErrorV4("JUDGE_FORMAL_BLIND_RUN_INVALID")
...
def _public_input(case: object, materialization: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(product_input, Mapping):
        raise P5JudgeErrorV4("JUDGE_PRODUCT_INPUT_INVALID")
    if input_kind == "TEXT":
        ...
        if not isinstance(lines, list) or not lines:
            raise P5JudgeErrorV4("JUDGE_SCREENSHOT_OCR_TEXT_MISSING")
        if any(
            not isinstance(line, Mapping) or not isinstance(line.get("text"), str)
            for line in lines
        ):
            raise P5JudgeErrorV4("JUDGE_SCREENSHOT_OCR_TEXT_INVALID")
        text = "\n".join(str(line["text"]) for line in lines)
    if not isinstance(text, str) or not text.strip():
        raise P5JudgeErrorV4("JUDGE_PUBLIC_INPUT_TEXT_INVALID")
    return {
...
def _candidate_expression(output: object) -> dict[str, Any]:
    ...
    if not isinstance(raw_advice, list):
        raise P5JudgeErrorV4("JUDGE_ADVICE_INVALID")
    ...
    if not isinstance(projection, Mapping):
        raise P5JudgeErrorV4("JUDGE_PROJECTION_INVALID")
    return {
...
def _evidence_summary(
    ...
    if not isinstance(findings, list) or not isinstance(postcheck, Mapping):
        raise P5JudgeErrorV4("JUDGE_EVIDENCE_SUMMARY_INVALID")
    return {
        "finding_summaries": [
            {
...
def _judge_rubric_projection(source: Mapping[str, Any]) -> dict[str, Any]:
    if (
        ...
        or source.get("judge_may_decide") != list(DIMENSIONS_V4)
        or not isinstance(source.get("dimensions"), Mapping)
        or set(source["dimensions"]) != set(DIMENSIONS_V4)
    ):
        raise P5JudgeErrorV4("JUDGE_RUBRIC_CONTRACT_INVALID")
    return {
        "schema_version": "trip-check-p5-judge-rubric-projection-v4",
        "judge_class": "automated_proxy_judge",
        "fact_authority": "DETERMINISTIC_SCORER_ONLY",
        "human_evidence": False,
        "human_calibration_performed": False,
        "judge_may_decide": list(DIMENSIONS_V4),
...
def _assert_anonymous_bundle(bundle: Mapping[str, Any]) -> None:
    ...
    if any(fragment in serialized for fragment in forbidden_fragments):
        raise P5JudgeErrorV4("JUDGE_INPUT_ANONYMITY_VIOLATION")


def export_judge_bundles_v4(
    *,
    repo_root: Path,
    run_dir: Path,
    ...
    rubric_path: Path,
    blind_run_validator: BlindRunValidatorV4 | None = None,
) -> dict[str, Any]:
    ...
    validator = blind_run_validator or _default_blind_run_validator_v4
    result = validator(run_dir=run_dir, repo_root=root, require_formal=True)
    manifest, cases, outputs, materializations = _normalize_validated_run_v4(result)
    _validate_blind_run_shape_v4(manifest, cases, outputs)
    ...
    for round_index in range(1, ROUND_COUNT_V4 + 1):
        round_items: list[dict[str, Any]] = []
        for case_index, case_id in enumerate(ordered_case_ids):
            shift = (case_index + round_index - 1) % len(VARIANT_IDS_V4)
            ordered_variants = VARIANT_IDS_V4[shift:] + VARIANT_IDS_V4[:shift]
    ...
    if len(mapping_rows) != BLIND_TERMINAL_COUNT_V4 * ROUND_COUNT_V4:
        raise P5JudgeErrorV4("JUDGE_MAPPING_COVERAGE_INVALID")
    for round_index in range(1, ROUND_COUNT_V4 + 1):
        counts = Counter(
        ...
        if set(counts.values()) != {BLIND_CASE_COUNT_V4 // 3}:
            raise P5JudgeErrorV4("JUDGE_PERMUTATION_UNBALANCED")
    ...
    for round_index, (round_dir, items) in enumerate(
        ...
        path = round_dir / f"judge_input_round_{round_index}.v4.json"
        if path.exists():
            raise P5JudgeErrorV4("JUDGE_BUNDLE_ALREADY_EXISTS")
        bundle = {
            "schema_version": "trip-check-p5-judge-bundle-v4",
            "round_index": round_index,
        ...
        bundle_receipts.append(
            {
                ...
                "item_count": BLIND_TERMINAL_COUNT_V4,
            }
        )

    custody_dir.mkdir(parents=True, exist_ok=True)
    mapping_path = custody_dir / "judge_variant_mapping.v4.json"
    if mapping_path.exists():
        raise P5JudgeErrorV4("JUDGE_MAPPING_ALREADY_EXISTS")
    mapping = {
        "schema_version": "trip-check-p5-judge-mapping-v4",
        "subject_commit": manifest["subject_commit"],
    ...
    return {
        "schema_version": "trip-check-p5-judge-export-receipt-v4",
        "lane": "frozen_blind",
        "case_count": BLIND_CASE_COUNT_V4,
        "items_per_round": BLIND_TERMINAL_COUNT_V4,
        "round_count": ROUND_COUNT_V4,
...
def _validate_round_report_v4(
    ...
    if (
        set(report) != allowed
        or report.get("schema_version") != "trip-check-p5-judge-round-v4"
        ...
        raise P5JudgeErrorV4("JUDGE_ROUND_CONTRACT_INVALID")
    for key in (
        "evaluator_id",
        "agent_task_id",
        "agent_id",
        ...
        if not isinstance(report.get(key), str) or not report[key]:
            raise P5JudgeErrorV4("JUDGE_ROUND_PROVENANCE_INVALID")
    scores = report.get("scores")
    if not isinstance(scores, list) or len(scores) != BLIND_TERMINAL_COUNT_V4:
        raise P5JudgeErrorV4("JUDGE_ROUND_SCORE_COUNT_INVALID")
    allowed_score = {
        "anonymous_item_id",
        "slot_id",
        *DIMENSIONS_V4,
    ...
    for score in scores:
        if not isinstance(score, dict) or set(score) != allowed_score:
            raise P5JudgeErrorV4("JUDGE_SCORE_SCHEMA_INVALID")
        dimensions = [score.get(dimension) for dimension in DIMENSIONS_V4]
        if any(
            ...
            raise P5JudgeErrorV4("JUDGE_SCORE_RANGE_INVALID")
        ...
        if (
            ...
            raise P5JudgeErrorV4("JUDGE_CLAIM_CANDIDATES_INVALID")
        if (
            ...
            raise P5JudgeErrorV4("JUDGE_SCORE_ID_INVALID")
        verdict = "PASS" if min(dimensions) >= 2 and not claims else "NEEDS_REVISION"
        if score.get("derived_verdict") != verdict:
            raise P5JudgeErrorV4("JUDGE_DERIVED_VERDICT_INVALID")
        keys.append((score["anonymous_item_id"], score["slot_id"]))
    if len(keys) != len(set(keys)):
        raise P5JudgeErrorV4("JUDGE_SCORE_KEY_DUPLICATE")
    return scores


def aggregate_judge_rounds_v4(
    *,
    repo_root: Path,
    mapping_path: Path,
    mapping_sha256: str,
    round_paths: Sequence[Path],
    minimum_agreement: float = JUDGE_AGREEMENT_THRESHOLD_V4,
) -> dict[str, Any]:
    """Aggregate exactly three independently-provenanced Judge rounds."""

    if minimum_agreement != JUDGE_AGREEMENT_THRESHOLD_V4:
        raise P5JudgeErrorV4("JUDGE_AGREEMENT_THRESHOLD_IMMUTABLE")
    root = repo_root.resolve()
    if (
        ...
        or len(round_paths) != ROUND_COUNT_V4
    ):
        raise P5JudgeErrorV4("JUDGE_MAPPING_OR_ROUND_COUNT_INVALID")
    ...
    if (
        ...
        or mapping.get("schema_version") != "trip-check-p5-judge-mapping-v4"
        or any(
        ...
        or len(receipts) != ROUND_COUNT_V4
        or not isinstance(rows, list)
        or len(rows) != BLIND_TERMINAL_COUNT_V4 * ROUND_COUNT_V4
        or digest(rows) != mapping.get("mapping_commitment")
    ):
        raise P5JudgeErrorV4("JUDGE_MAPPING_CONTRACT_INVALID")
    ...
    for round_index, receipt in enumerate(receipts, 1):
        if (
            ...
            or receipt.get("item_count") != BLIND_TERMINAL_COUNT_V4
            ...
            raise P5JudgeErrorV4("JUDGE_BUNDLE_RECEIPT_INVALID")
    ...
    if any(
        ...
        or row.get("variant_id") not in VARIANT_IDS_V4
        or not isinstance(row.get("runtime_generator_model"), str)
        or not isinstance(row.get("claim_ids"), list)
        for row in rows
    ):
        raise P5JudgeErrorV4("JUDGE_MAPPING_ROW_INVALID")
    mapping_by_key = {
        (row["round_index"], row["anonymous_item_id"], row["slot_id"]): row
        for row in rows
    }
    if len(mapping_by_key) != len(rows):
        raise P5JudgeErrorV4("JUDGE_MAPPING_KEY_SET_INVALID")
    anonymous_ids = {row["anonymous_item_id"] for row in rows}
    if len(anonymous_ids) != BLIND_CASE_COUNT_V4:
        raise P5JudgeErrorV4("JUDGE_ANONYMOUS_MAPPING_NOT_CLOSED")
    for round_index in range(1, ROUND_COUNT_V4 + 1):
        ...
        if (
            len(round_rows) != BLIND_TERMINAL_COUNT_V4
            or {(row["anonymous_item_id"], row["slot_id"]) for row in round_rows}
            != expected_keys
        ):
            raise P5JudgeErrorV4("JUDGE_MAPPING_ROUND_COVERAGE_INVALID")
    ...
    for path in round_paths:
        if not path.is_absolute() or _inside(path, root) or _contains_link(path.absolute()):
            raise P5JudgeErrorV4("JUDGE_ROUND_CUSTODY_INVALID")
        ...
        if not isinstance(round_index, int) or round_index not in {1, 2, 3}:
            raise P5JudgeErrorV4("JUDGE_ROUND_INDEX_INVALID")
        report["scores"] = _validate_round_report_v4(
    ...
    if len({report["round_index"] for report in reports}) != 3 or any(
        ...
        raise P5JudgeErrorV4("JUDGE_ROUND_INDEPENDENCE_INVALID")
    ...
    for report in reports:
        for score in report["scores"]:
            ...
            if row is None:
                raise P5JudgeErrorV4("JUDGE_SCORE_MAPPING_MISSING")
            if not set(score["unsupported_claim_candidate_ids"]).issubset(
                row["claim_ids"]
            ):
                raise P5JudgeErrorV4("JUDGE_CLAIM_ID_NOT_IN_CANDIDATE")
            if report["model_id"] == row["runtime_generator_model"]:
                raise P5JudgeErrorV4("JUDGE_RUNTIME_SELF_REVIEW")
            grouped[(score["anonymous_item_id"], row["variant_id"])].append(score)
    if len(grouped) != BLIND_TERMINAL_COUNT_V4 or any(
        len(values) != 3 for values in grouped.values()
    ):
        raise P5JudgeErrorV4("JUDGE_VARIANT_COVERAGE_INVALID")
    ...
    for (_, variant_id), values in grouped.items():
        ...
        per_dimension = {
            ...
            <= 1
            for dimension in DIMENSIONS_V4
        }
    ...
    panel = {
        ...
        "human_calibration_performed": False,
        "round_count": ROUND_COUNT_V4,
        "candidate_count": BLIND_TERMINAL_COUNT_V4,
...
__all__ = [
    ...
    "JUDGE_AGREEMENT_THRESHOLD_V4",
    "P5JudgeErrorV4",
    "aggregate_judge_rounds_v4",
    "export_judge_bundles_v4",
]
