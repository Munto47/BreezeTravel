"""Fail-closed P5 v4 Evaluation Gate with same-subject artifact readback."""
...
class P5GateErrorV4(RuntimeError):
    """Stable fail-closed P5 v4 gate error."""
...
def _load_json(path: Path, reason: str) -> dict[str, Any]:
    try:
        ...
        raise P5GateErrorV4(reason) from exc
    if not isinstance(value, dict):
        raise P5GateErrorV4(reason)
    return value
...
def _sha256(path: Path) -> str:
    try:
        ...
        raise P5GateErrorV4("GATE_ARTIFACT_UNREADABLE") from exc
...
def _repo_state_v4(repo_root: Path) -> dict[str, Any]:
    def run(*args: str) -> str:
        try:
            ...
            raise P5GateErrorV4("GATE_REPOSITORY_STATE_UNAVAILABLE") from exc
...
def _parse_dataset_v4(
    *, repo_root: Path, path: Path, run_spec_path: Path, rubric_path: Path
) -> dict[str, Any]:
    value = _load_json(path, "V4_DATASET_MANIFEST_INVALID")
    _validate_self_hash(value, "manifest_hash", "V4_DATASET_MANIFEST_HASH_MISMATCH")
    required = {
        "schema_version",
        "dataset_id",
        "manifest_hash",
    ...
    _require_fields(value, required, "V4_DATASET_MANIFEST_FIELDS_MISSING")
    ...
    if (
        value.get("schema_version") != "trip-check-p5-dataset-manifest-v4"
        ...
        or not _is_sha256(contracts.get("dataset_contracts_v4_sha256"))
    ):
        raise P5GateErrorV4("V4_DATASET_FORMAL_CONTRACT_INVALID")
    expected_rows = {
        "nonblind_cases": 270,
        "blind_cases": 90,
    ...
    for key, row_count in expected_rows.items():
        ...
        if not isinstance(entry, Mapping):
            raise P5GateErrorV4("V4_DATASET_FILE_BINDING_INVALID")
        ...
        try:
            ...
            raise P5GateErrorV4("V4_DATASET_FILE_BINDING_INVALID") from exc
        if (
            not source.is_file()
...
def _parse_active_and_seal_v4(
    *,
    ...
    active = _load_json(active_path, "V4_ACTIVE_CONTRACT_INVALID")
    seal = _load_json(seal_path, "V4_BLIND_SEAL_INVALID")
    ...
    if (
        ...
        or seal.get("dataset_contracts_v4_sha256")
        != dataset["contract_hashes"]["dataset_contracts_v4_sha256"]
    ):
        raise P5GateErrorV4("V4_ACTIVE_SEAL_BINDING_INVALID")
    return active, seal


def _parse_run_manifest_v4(
    ...
    _require_fields(value, required, "V4_RUN_MANIFEST_FIELDS_MISSING")
    _validate_self_hash(value, "manifest_hash", "V4_RUN_MANIFEST_HASH_MISMATCH")
    expected_cases, expected_terminals = (
        (270, 810) if lane == "nonblind" else (90, 270)
    )
    if (
        value.get("schema_version") != "trip-check-p5-run-group-v4"
...
def parse_nonblind_score_v4(
    value: Mapping[str, Any], *, run: Mapping[str, Any]
) -> dict[str, Any]:
    """Strict, isolated parser for the independently-integrated v4 scorer."""
    ...
    _require_fields(value, required, "V4_NONBLIND_SCORE_FIELDS_MISSING")
    _validate_self_hash(value, "report_hash", "V4_NONBLIND_SCORE_HASH_MISMATCH")
    ...
    if (
        value.get("schema_version") != "trip-check-p5-nonblind-score-report-v4"
        ...
        or set(variants) != VARIANT_IDS_V4
        ...
        or decision not in PROMOTION_DECISIONS_V4
        ...
        raise P5GateErrorV4("V4_NONBLIND_SCORE_CONTRACT_INVALID")
    if value["status"] == "PASS" and (
        not all(zero_checks.values()) or not all(stage_checks.values())
    ):
        raise P5GateErrorV4("V4_NONBLIND_SCORE_STATUS_INVALID")
    if decision == "PROMOTE_ADMITTED_CHALLENGER":
        ...
        if challenger == "solver_c":
            raise P5GateErrorV4("V4_SOLVER_PROMOTION_FORBIDDEN")
        if (
            challenger not in VARIANT_IDS_V4 - {"core_b", "solver_c"}
            or not isinstance(admission, Mapping)
            or admission.get("status") != "ADMITTED"
            or admission.get("variant_id") != challenger
        ):
            raise P5GateErrorV4("V4_CHALLENGER_ADMISSION_INVALID")
    return dict(value)


def parse_blind_score_v4(
    ...
    _require_fields(value, required, "V4_BLIND_SCORE_FIELDS_MISSING")
    _validate_self_hash(value, "report_hash", "V4_BLIND_SCORE_HASH_MISMATCH")
    bindings = value.get("bindings")
    variants = value.get("variant_metrics")
    zero_checks = value.get("zero_tolerance_checks")
    if (
        value.get("schema_version") != "trip-check-p5-isolated-blind-score-v4"
        or value.get("status") not in {"PASS", "REJECT"}
        ...
        raise P5GateErrorV4("V4_BLIND_SCORE_CONTRACT_INVALID")
    ...
    if '"case_id"' in serialized or "p5.blind." in serialized or '"by_' in serialized:
        raise P5GateErrorV4("V4_BLIND_AGGREGATE_DETAIL_LEAK")
    if value["status"] == "PASS" and not all(zero_checks.values()):
        raise P5GateErrorV4("V4_BLIND_SCORE_STATUS_INVALID")
    return dict(value)


def _parse_judge_panel_v4(
    value: Mapping[str, Any], *, run: Mapping[str, Any]
) -> dict[str, Any]:
    required = {
        "schema_version",
    ...
    _require_fields(value, required, "V4_JUDGE_PANEL_FIELDS_MISSING")
    _validate_self_hash(value, "report_hash", "V4_JUDGE_PANEL_HASH_MISMATCH")
    provenance = value.get("provenance")
    dimension_agreement = value.get("per_dimension_agreement_rate")
    if (
        value.get("schema_version") != "trip-check-p5-judge-panel-v4"
        ...
        raise P5GateErrorV4("V4_JUDGE_PANEL_CONTRACT_INVALID")
    ...
    if (value["status"] == "PASS") != semantic_pass:
        raise P5GateErrorV4("V4_JUDGE_PANEL_STATUS_INVALID")
    return dict(value)


def _parse_formal_receipt_v4(
    *,
    repo_root: Path,
    ...
    _require_fields(value, required, "V4_FORMAL_RECEIPT_FIELDS_MISSING")
    _validate_self_hash(value, "receipt_hash", "V4_FORMAL_RECEIPT_HASH_MISMATCH")
    bindings = value.get("bindings")
    counts = value.get("counts")
    ...
    for kind in VERIFICATION_KINDS_V4:
        ...
        if (
            ...
            != "trip-check-p5-verification-receipt-v4"
...
def build_p5_gate_manifest_v4(
    *,
    ...
    active, _seal = _parse_active_and_seal_v4(
    ...
    nonblind_run = _parse_run_manifest_v4(
        _load_json(nonblind_run_manifest_path, "V4_NONBLIND_RUN_INVALID"),
        lane="nonblind",
        dataset_hash=dataset["manifest_hash"],
    )
    blind_run = _parse_run_manifest_v4(
        _load_json(blind_run_manifest_path, "V4_BLIND_RUN_INVALID"),
        lane="frozen_blind",
        dataset_hash=dataset["manifest_hash"],
    )
    ...
    nonblind_score = parse_nonblind_score_v4(
        _load_json(nonblind_score_path, "V4_NONBLIND_SCORE_INVALID"),
        run=nonblind_run,
    )
    blind_score = parse_blind_score_v4(
        _load_json(blind_score_path, "V4_BLIND_SCORE_INVALID"),
        run=blind_run,
        seal_sha256=_sha256(blind_seal_path),
    )
    panel = _parse_judge_panel_v4(
        _load_json(judge_panel_path, "V4_JUDGE_PANEL_INVALID"), run=blind_run
    )
    ...
    formal, verification_paths = _parse_formal_receipt_v4(
        repo_root=root,
        value=_load_json(formal_receipt_path, "V4_FORMAL_RECEIPT_INVALID"),
    ...
    if require_current_subject:
        ...
        if state != {
            ...
            raise P5GateErrorV4("V4_CURRENT_SUBJECT_UPSTREAM_OR_TREE_INVALID")
    exact_counts = (
    ...
    if decision not in PROMOTION_DECISIONS_V4:
        raise P5GateErrorV4("V4_PROMOTION_DECISION_INVALID")
    ...
    gate_accepted = quality_pass and decision != "REJECT_ALL" and all(checks.values())
    manifest = {
        "schema_version": "trip-check-p5-evaluation-gate-v4",
    ...
    schema = _load_json(gate_schema_path, "V4_GATE_SCHEMA_INVALID")
    errors = sorted(
