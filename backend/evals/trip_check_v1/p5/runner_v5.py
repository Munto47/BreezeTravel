"""One-shot, repository-external runner for the sealed P5 v5 blind lane.

The v5 envelope supersedes only the invalid v4 oracle commitment.  Blind
inputs/materializations retain the frozen v4 bytes (and therefore the v3
payload contract), while product execution stays on the admitted v4 adapters.
Imports remain delayed so dataset/seal custody can be integrated independently.
"""
...
BLIND_SCREENSHOT_HASH_COUNT_V5 = 45
BLIND_OCR_REPLAY_COUNT_V5 = 180
RUN_GROUP_SCHEMA_V5 = "trip-check-p5-blind-run-group-v5"
NONCE_SCHEMA_V5 = "trip-check-p5-blind-run-nonce-v5"
CONSUMPTION_SCHEMA_V5 = "trip-check-p5-blind-run-consumption-receipt-v5"
_RUN_ID = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]{0,127}\Z")
_HEX_64 = re.compile(r"[0-9a-f]{64}\Z")


class P5BlindRunnerErrorV5(RuntimeError):
...
def _fail(reason_code: str, exc: Exception | None = None) -> None:
    if exc is None:
        raise P5BlindRunnerErrorV5(reason_code)
    raise P5BlindRunnerErrorV5(reason_code) from exc
...
class BlindExecutionEngineV5(Protocol):
    async def execute(
        self,
        *,
...
# These two envelope models live beside the executor because v5 intentionally
...
class P5VariantRunSpecV5(P5VariantRunSpecV3):
    model_config = ConfigDict(frozen=True, extra="forbid")

    schema_version: Literal["trip-check-p5-variant-run-spec-v5"] = (
        "trip-check-p5-variant-run-spec-v5"
    )
    replay_hash_policy: Literal["p5-semantic-projection-v5"] = (
        "p5-semantic-projection-v5"
    )


class P5TerminalOutputV5(P5TerminalOutputV3):
    model_config = ConfigDict(frozen=True, extra="forbid")

    schema_version: Literal["trip-check-p5-terminal-output-v5"] = (
        "trip-check-p5-terminal-output-v5"
    )


def build_run_spec_v5(
    *,
    ...
    """Build one lane-neutral v5 RunSpec over the unchanged v4 adapters."""
    ...
    if variant_id not in VARIANT_IDS_V5 or set(adapter_versions) != set(VARIANT_IDS_V5):
        _fail("P5_V5_ADAPTER_SET_INVALID")
    adapter_version, repair_strategy = adapter_versions[variant_id]
    if not adapter_version.endswith("-v4"):
        _fail("P5_V5_ADAPTER_VERSION_INVALID")
    if template.get("variant_specs", {}).get(variant_id) != {
        "adapter_version": adapter_version,
        "repair_strategy": repair_strategy,
    }:
        _fail("P5_V5_RUN_SPEC_ADAPTER_BINDING_INVALID")
    historical_ocr = template.get("historical_ocr_evidence")
    if not isinstance(historical_ocr, Mapping):
        _fail("P5_V5_RUN_SPEC_OCR_POLICY_MISSING")
    if template.get("replay_hash_policy") != "p5-semantic-projection-v5":
        _fail("P5_V5_REPLAY_HASH_POLICY_INVALID")
    return P5VariantRunSpecV5(
        ...
        replay_hash_policy="p5-semantic-projection-v5",
...
def _semantic_payload_v5(output: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "replay_hash_policy": "p5-semantic-projection-v5",
...
def semantic_output_hash_v5(output: P5TerminalOutputV5 | Mapping[str, Any]) -> str:
    payload = (
        output.model_dump(mode="json")
        if isinstance(output, P5TerminalOutputV5)
        else dict(output)
    )
    return digest(_semantic_payload_v5(payload))


async def execute_terminal_v5(
    *,
    case: Any,
    materialization: Mapping[str, Any],
    run_spec: P5VariantRunSpecV5,
    adapter: Any,
) -> P5TerminalOutputV5:
    """Execute one v5 terminal in either lane with v5-only semantic hashing."""

    from evals.trip_check_v1.p5.adapters_v2 import _HarnessResult
    from evals.trip_check_v1.p5.contracts_v3 import P5CaseV3
    from evals.trip_check_v1.p5.data_contract_v5 import validate_materialization_v5
    ...
    if (
        ...
        _fail("P5_V5_ADAPTER_RUN_SPEC_MISMATCH")
    if materialization.get("case_id") != validated_case.case_id:
        _fail("P5_V5_CASE_MATERIALIZATION_ID_MISMATCH")
    try:
        validated_materialization = validate_materialization_v5(
            validated_case, materialization
        )
    except Exception as exc:
        _fail("P5_V5_MATERIALIZATION_INVALID", exc)
    ...
    try:
        ...
        result = _HarnessResult(
            ...
            advice=[],
            postcheck=None,
            receipts=[{"type": "runner_timeout", "contract": "v5"}],
            raw_artifact={},
        )
    except Exception as exc:
    ...
    semantic_hash = digest(_semantic_payload_v5(terminal_payload))
    terminal_payload["semantic_output_hash"] = semantic_hash
    terminal_payload["replay_hash"] = semantic_hash
    return P5TerminalOutputV5.model_validate(terminal_payload)


class V3PayloadBlindExecutionEngineV5:
    """Execute the unchanged v4/v3 payload through frozen v4 product adapters."""
    ...
    async def execute(
        ...
        *,
        ...
        # Delayed by design: the v5 envelope does not own these payload contracts.
        ...
        for case in cases:
            try:
                materialization = validate_materialization_v5(
...
def _validate_dataset_envelope(
    *, paths: BlindDatasetPathsV5
    ...
    if (
        seal.get("schema_version") != "trip-check-p5-blind-seal-v5"
        or seal.get("split") != "frozen_blind"
        or seal.get("case_count") != BLIND_CASE_COUNT_V5
    ...
    if (
        ...
        or active.get("active_contract") != ACTIVE_CONTRACT_V5
        or active.get("formal_evidence_status") != "READY"
        or active.get("dataset_manifest_hash") != manifest["manifest_hash"]
        or active.get("blind_seal_v5_sha256") != file_sha256(paths.seal)
    ):
        _fail("P5_V5_FORMAL_CONTRACT_NOT_READY")
    ...
    if not isinstance(commitment, Mapping) or (
        ...
        or any(
            _HEX_64.fullmatch(str(seal.get(field))) is None
            for field in (
        ...
        or commitment.get("review_receipt_sha256")
        != seal.get("review_receipt_sha256")
    ):
    ...
    if (
        len(inputs) != BLIND_CASE_COUNT_V5
        or len(materializations) != BLIND_CASE_COUNT_V5
        or len(set(case_ids)) != BLIND_CASE_COUNT_V5
        or len(set(materialization_ids)) != BLIND_CASE_COUNT_V5
    ...
    if (
        lane.get("case_count") != BLIND_CASE_COUNT_V5
        or lane.get("materialization_count") != BLIND_CASE_COUNT_V5
    ...
    if (
        ...
        or seal.get("variant_ids_sha256") != digest(list(VARIANT_IDS_V5))
    ):
...
def _claim_nonce(
    *, nonce_file: Path, consumption_dir: Path, dataset_manifest_hash: str
    ...
    if set(nonce_envelope) != {
        ...
        nonce_envelope.get("schema_version") != NONCE_SCHEMA_V5
        or nonce_envelope.get("purpose") != "execute_frozen_blind_once"
        or nonce_envelope.get("dataset_id") != DATASET_ID_V5
        or nonce_envelope.get("active_contract") != ACTIVE_CONTRACT_V5
    ...
    claim = {
        "schema_version": CONSUMPTION_SCHEMA_V5,
        "status": "CLAIMED",
...
def _terminal_key(row: Mapping[str, Any]) -> tuple[str, str]:
    ...
    if not isinstance(case_id, str) or variant_id not in VARIANT_IDS_V5:
        _fail("BLIND_TERMINAL_KEY_INVALID")
    return case_id, str(variant_id)


def _validate_execution_result(result: BlindExecutionResultV5) -> list[dict[str, str]]:
    terminals = list(result.terminals)
    replays = list(result.replay_terminals)
    if len(terminals) != BLIND_TERMINAL_COUNT_V5 or len(replays) != BLIND_TERMINAL_COUNT_V5:
    ...
    if (
        len(terminal_by_key) != BLIND_TERMINAL_COUNT_V5
        or len(replay_by_key) != BLIND_TERMINAL_COUNT_V5
        or set(terminal_by_key) != set(replay_by_key)
        or set(result.run_specs) != set(VARIANT_IDS_V5)
    ):
    ...
    for key in sorted(terminal_by_key):
        ...
        semantic_valid = (
            terminal.get("semantic_output_hash") == semantic_output_hash_v5(terminal)
            and replay_terminal.get("semantic_output_hash")
            == semantic_output_hash_v5(replay_terminal)
        )
    ...
    expected_ocr = {
        "lookup_count": BLIND_OCR_REPLAY_COUNT_V5,
        "hit_count": BLIND_OCR_REPLAY_COUNT_V5,
        "receipt_match_count": BLIND_OCR_REPLAY_COUNT_V5,
        "cleanup_deleted_count": BLIND_OCR_REPLAY_COUNT_V5,
        "miss_count": 0,
        "fallback_count": 0,
        "fresh_prediction_count": 0,
        "unique_hash_count": BLIND_SCREENSHOT_HASH_COUNT_V5,
    }
    if len(result.screenshot_hashes) != BLIND_SCREENSHOT_HASH_COUNT_V5 or any(
        result.ocr_provenance.get(key) != value for key, value in expected_ocr.items()
    ) or terminal_provenance_count != BLIND_OCR_REPLAY_COUNT_V5:
        _fail("BLIND_OCR_REPLAY_PROVENANCE_INVALID")
    return mismatches


async def run_blind_once_v5(
    *,
    repo_root: Path,
    ...
    upstream_commit: str,
    dirty_tree: bool,
    engine: BlindExecutionEngineV5 | None = None,
) -> dict[str, Any]:
    ...
    try:
        ...
        execution = await (engine or V3PayloadBlindExecutionEngineV5()).execute(
            case_rows=inputs,
        ...
        artifact_index = {
            "schema_version": "trip-check-p5-blind-artifact-index-v5",
            "subject_commit": subject_commit,
            "dirty_tree": False,
            "entries": artifacts,
        }
        ...
        manifest_core: dict[str, Any] = {
            "schema_version": RUN_GROUP_SCHEMA_V5,
            "run_id": run_id,
            "status": "PASS",
            ...
            "variant_count": len(VARIANT_IDS_V5),
            "run_specs": dict(execution.run_specs),
            "terminal_count": BLIND_TERMINAL_COUNT_V5,
            "expected_terminal_count": BLIND_TERMINAL_COUNT_V5,
            ...
            "replay_readback_count": BLIND_TERMINAL_COUNT_V5,
            ...
            "ocr_replay_provenance": {
                **dict(execution.ocr_provenance),
                ...
                "terminal_provenance_count": BLIND_OCR_REPLAY_COUNT_V5,
                "expected_formal_lookup_count": BLIND_OCR_REPLAY_COUNT_V5,
            },
        ...
        if isinstance(exc, P5BlindRunnerErrorV5):
            raise
...
def validate_blind_run_group_v5(
    *,
    run_dir: Path,
    repo_root: Path,
    require_formal: bool = True,
    dataset_paths: BlindDatasetPathsV5 | None = None,
    ...
    if (
        run_manifest.get("schema_version") != RUN_GROUP_SCHEMA_V5
        or run_manifest.get("lane") != "frozen_blind"
        or run_manifest.get("status") != "PASS"
        or run_manifest.get("dataset_id") != DATASET_ID_V5
        or run_manifest.get("variant_ids") != list(VARIANT_IDS_V5)
        or run_manifest.get("variant_count") != len(VARIANT_IDS_V5)
        or run_manifest.get("case_count") != BLIND_CASE_COUNT_V5
        or run_manifest.get("terminal_count") != BLIND_TERMINAL_COUNT_V5
        ...
        or run_manifest.get("replay_hash_policy") != "p5-semantic-projection-v5"
    ...
    try:
        ...
        outputs = [P5TerminalOutputV5.model_validate(row) for row in terminal_rows]
        replay_outputs = [P5TerminalOutputV5.model_validate(row) for row in replay_rows]
    except Exception as exc:
    ...
    for row in raw_materializations:
        ...
        try:
            materialization_by_id[case_id] = validate_materialization_v5(
    ...
    if (
        len(output_by_key) != BLIND_TERMINAL_COUNT_V5
...
__all__ = [
    ...
    "BLIND_SCREENSHOT_HASH_COUNT_V5",
    "BLIND_TERMINAL_COUNT_V5",
    "BlindDatasetPathsV5",
    "BlindExecutionEngineV5",
    "BlindExecutionResultV5",
    "DATASET_ID_V5",
    "P5BlindRunnerErrorV5",
    "V3PayloadBlindExecutionEngineV5",
    "VARIANT_IDS_V5",
    "run_blind_once_v5",
