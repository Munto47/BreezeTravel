"""Formal repository-external runner and readback for P5 v5 non-blind data."""
...
NONBLIND_CASE_COUNT_V5 = 270
NONBLIND_TERMINAL_COUNT_V5 = 810
NONBLIND_SCREENSHOT_HASH_COUNT_V5 = 126
NONBLIND_OCR_LOOKUP_COUNT_V5 = 504
RUN_GROUP_SCHEMA_V5 = "trip-check-p5-run-group-v5"
ARTIFACT_INDEX_SCHEMA_V5 = "trip-check-p5-nonblind-artifact-index-v5"
_RUN_ID = re.compile(r"[A-Za-z0-9][A-Za-z0-9._-]{0,127}\Z")


class P5NonblindRunnerErrorV5(RuntimeError):
    """Stable fail-closed validation error for the v5 non-blind lane."""
...
def _fail(reason_code: str, exc: Exception | None = None) -> None:
    if exc is None:
        raise P5NonblindRunnerErrorV5(reason_code)
    raise P5NonblindRunnerErrorV5(reason_code) from exc
...
@dataclass(frozen=True)
class NonblindExecutionResultV5:
...
class NonblindExecutionEngineV5(Protocol):
    async def execute(
        self,
        *,
    ...
    ) -> NonblindExecutionResultV5: ...


class V3PayloadNonblindExecutionEngineV5:
    """Run v3 payloads through v5 RunSpecs/adapters with no oracle access."""
    ...
    async def execute(
        ...
        *,
        ...
        for variant_id in VARIANT_IDS_V5:
            ...
            for case in cases:
                for target in (terminals, replays):
                    output = await execute_terminal_v5(
                        case=case,
        ...
        return NonblindExecutionResultV5(
...
def _default_dataset_paths() -> NonblindDatasetPathsV5:
    ...
    return NonblindDatasetPathsV5(
        cases=NONBLIND_PATH_V5,
        materializations=NONBLIND_MATERIALIZATIONS_PATH_V5,
...
def _validate_dataset_envelope(
    *, paths: NonblindDatasetPathsV5, require_formal: bool
) -> tuple[
    dict[str, Any],
    dict[str, Any] | None,
    dict[str, Any] | None,
    ...
    if (
        len(cases) != NONBLIND_CASE_COUNT_V5
        or len(materializations) != NONBLIND_CASE_COUNT_V5
        or len(set(case_ids)) != NONBLIND_CASE_COUNT_V5
        or len(set(materialization_ids)) != NONBLIND_CASE_COUNT_V5
        ...
        or lane.get("case_count") != NONBLIND_CASE_COUNT_V5
        or lane.get("materialization_count") != NONBLIND_CASE_COUNT_V5
    ):
    ...
    if not isinstance(contract_hashes, Mapping) or (
        ...
        or template.get("schema_version") != "trip-check-p5-run-spec-v5"
        or template.get("replay_hash_policy") != "p5-semantic-projection-v5"
...
def _validate_execution_result(
    result: NonblindExecutionResultV5,
    *,
    ...
    if (
        len(terminals) != NONBLIND_TERMINAL_COUNT_V5
        or len(replays) != NONBLIND_TERMINAL_COUNT_V5
        or len(terminal_by_key) != NONBLIND_TERMINAL_COUNT_V5
        or len(replay_by_key) != NONBLIND_TERMINAL_COUNT_V5
    ...
    try:
        specs = {
            variant_id: P5VariantRunSpecV5.model_validate(
    ...
    for variant_id, spec in specs.items():
        if (
            ...
            or spec.replay_hash_policy != "p5-semantic-projection-v5"
    ...
    for key in sorted(terminal_by_key):
        ...
        if (
            ...
            or first.get("semantic_output_hash") != semantic_output_hash_v5(first)
            or replay.get("semantic_output_hash") != semantic_output_hash_v5(replay)
        ...
        for row in (first, replay):
            try:
                P5TerminalOutputV5.model_validate(row)
            except Exception as exc:
    ...
    expected_ocr = {
        "lookup_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
        "hit_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
        "receipt_match_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
        "cleanup_deleted_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
        "miss_count": 0,
        "fallback_count": 0,
        "fresh_prediction_count": 0,
        "unique_hash_count": NONBLIND_SCREENSHOT_HASH_COUNT_V5,
    }
    if (
        set(result.screenshot_hashes) != expected_screenshot_hashes
        or len(result.screenshot_hashes) != NONBLIND_SCREENSHOT_HASH_COUNT_V5
        or terminal_provenance_count != NONBLIND_OCR_LOOKUP_COUNT_V5
...
async def run_nonblind_v5(
    *,
    ...
    upstream_commit: str,
    dirty_tree: bool,
    dataset_paths: NonblindDatasetPathsV5 | None = None,
    engine: NonblindExecutionEngineV5 | None = None,
    ...
    execution = await (engine or V3PayloadNonblindExecutionEngineV5()).execute(
        case_rows=cases,
    ...
    artifact_index = {
        "schema_version": ARTIFACT_INDEX_SCHEMA_V5,
        "subject_commit": subject_commit,
        "dirty_tree": False,
        "entries": entries,
    }
    ...
    run_manifest: dict[str, Any] = {
        ...
        "case_count": NONBLIND_CASE_COUNT_V5,
        ...
        "terminal_count": NONBLIND_TERMINAL_COUNT_V5,
        "expected_terminal_count": NONBLIND_TERMINAL_COUNT_V5,
        ...
        "replay_match_count": NONBLIND_TERMINAL_COUNT_V5,
        "replay_readback_count": NONBLIND_TERMINAL_COUNT_V5,
        "replay_mismatches": [],
        ...
        "ocr_replay_provenance": {
            **dict(execution.ocr_provenance),
            ...
            "terminal_provenance_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
            "expected_formal_lookup_count": NONBLIND_OCR_LOOKUP_COUNT_V5,
        },
...
def validate_nonblind_run_group_v5(
    *,
    run_dir: Path,
    repo_root: Path,
    require_formal: bool = True,
    dataset_paths: NonblindDatasetPathsV5 | None = None,
) -> tuple[dict[str, Any], list[Any], list[P5TerminalOutputV5], dict[str, dict[str, Any]]]:
    """Validate active/seal/git/dataset/artifacts before v5 scoring."""
    ...
    if (
        ...
        or run_manifest.get("case_count") != NONBLIND_CASE_COUNT_V5
        or run_manifest.get("terminal_count") != NONBLIND_TERMINAL_COUNT_V5
        or run_manifest.get("expected_terminal_count")
        != NONBLIND_TERMINAL_COUNT_V5
        or run_manifest.get("replay_executed") is not True
        or run_manifest.get("replay_match_count") != NONBLIND_TERMINAL_COUNT_V5
    ...
    if (
        artifact_index.get("schema_version") != ARTIFACT_INDEX_SCHEMA_V5
    ...
    if (
        len(output_by_key) != NONBLIND_TERMINAL_COUNT_V5
        ...
        or any(
            ...
            != semantic_output_hash_v5(output_by_key[key])
            or replay_by_key[key].semantic_output_hash
            != semantic_output_hash_v5(replay_by_key[key])
...
__all__ = [
    "NONBLIND_CASE_COUNT_V5",
    "NONBLIND_OCR_LOOKUP_COUNT_V5",
    "NONBLIND_SCREENSHOT_HASH_COUNT_V5",
    "NONBLIND_TERMINAL_COUNT_V5",
    "NonblindDatasetPathsV5",
    "NonblindExecutionEngineV5",
    "NonblindExecutionResultV5",
    "P5NonblindRunnerErrorV5",
    "V3PayloadNonblindExecutionEngineV5",
    "run_nonblind_v5",
    "validate_nonblind_run_group_v5",
]
