"""Executable receipt chain for the formal P5 v4 Evaluation Gate.
...
COMMAND_KINDS_V4 = ("dataset_formal", *VERIFICATION_KINDS_V4)
COMMAND_STATUSES_V4 = {"PASS", "FAIL", "BLOCKED"}
RECEIPT_STATUSES_V4 = {"PASS", "FAIL", "BLOCKED", "NOT_RUN", "REJECT"}
PRIMARY_ARTIFACT_NAMES_V4 = (
    "dataset_manifest",
    "active_contract",
    "blind_seal",
    "run_spec",
...
class P5FormalReceiptErrorV4(RuntimeError):
...
def _git(repo_root: Path, *args: str) -> str:
    try:
        ...
        raise P5FormalReceiptErrorV4("RECEIPT_GIT_STATE_UNAVAILABLE") from exc


def read_repo_binding_v4(repo_root: Path) -> RepoBindingV4:
    return RepoBindingV4(
...
def _require_formal_repo_binding(binding: RepoBindingV4) -> None:
    if (
        ...
        raise P5FormalReceiptErrorV4("RECEIPT_SUBJECT_NOT_CLEAN_PUSHED_UPSTREAM")
...
def _sha256(path: Path) -> str:
    try:
        ...
        raise P5FormalReceiptErrorV4("RECEIPT_ARTIFACT_UNREADABLE") from exc
...
def _load_json(path: Path, reason: str) -> dict[str, Any]:
    try:
        ...
        raise P5FormalReceiptErrorV4(reason) from exc
    if not isinstance(value, dict):
        raise P5FormalReceiptErrorV4(reason)
    return value
...
def require_external_output_v4(repo_root: Path, path: Path) -> Path:
    if not path.is_absolute() or _inside(path.absolute(), repo_root) or _contains_link(
        path.absolute()
    ):
        raise P5FormalReceiptErrorV4("RECEIPT_OUTPUT_MUST_BE_EXTERNAL")
...
def _artifact(logical_name: str, path: Path) -> dict[str, Any]:
    ...
    if not resolved.is_file() or _contains_link(resolved):
        raise P5FormalReceiptErrorV4("RECEIPT_ARTIFACT_PATH_INVALID")
    return {
...
def _parse_named_paths(values: dict[str, Path]) -> list[dict[str, Any]]:
    if not values or any(not name or not path.is_absolute() for name, path in values.items()):
        raise P5FormalReceiptErrorV4("RECEIPT_NAMED_ARTIFACTS_INVALID")
...
def execute_command_receipt_v4(
    *,
    repo_root: Path,
    kind: str,
    command: list[str],
    ...
    output_dir: Path,
    repo_binding: RepoBindingV4 | None = None,
) -> dict[str, Any]:
    """Run one real command and persist its immutable result/log receipt."""

    if kind not in COMMAND_KINDS_V4 or not command:
        raise P5FormalReceiptErrorV4("COMMAND_RECEIPT_ARGUMENT_INVALID")
    root = repo_root.resolve()
    destination = require_external_output_v4(root, output_dir)
    if destination.exists() and any(destination.iterdir()):
        raise P5FormalReceiptErrorV4("COMMAND_RECEIPT_OUTPUT_NOT_EMPTY")
    destination.mkdir(parents=True, exist_ok=True)
    try:
        cwd = command_cwd.resolve(strict=True)
        cwd.relative_to(root)
    except (OSError, ValueError) as exc:
        raise P5FormalReceiptErrorV4("COMMAND_RECEIPT_CWD_INVALID") from exc
    before = repo_binding or read_repo_binding_v4(root)
    ...
    for name, path in sorted(artifact_paths.items()):
        ...
        except (OSError, P5FormalReceiptErrorV4):
            artifact_errors.append(f"ARTIFACT_READBACK_FAILED:{name}")
    after = repo_binding or read_repo_binding_v4(root)
    ...
    receipt: dict[str, Any] = {
        "schema_version": "trip-check-p5-command-result-v4",
        "receipt_kind": kind,
        "status": status,
        **before.as_dict(),
        "command": command,
    ...
    receipt_path = destination / "command_result.v4.json"
    _atomic_json(receipt_path, receipt)
    validate_command_result_v4(receipt_path)
    return {**receipt, "receipt_path": str(receipt_path)}


def validate_command_result_v4(path: Path) -> dict[str, Any]:
    ...
    if (
        set(value) != expected
        or value.get("schema_version") != "trip-check-p5-command-result-v4"
        or value.get("receipt_kind") not in COMMAND_KINDS_V4
        or value.get("status") not in COMMAND_STATUSES_V4
        ...
        raise P5FormalReceiptErrorV4("COMMAND_RESULT_CONTRACT_INVALID")
    ...
    if (value["status"] == "PASS") != pass_shape:
        raise P5FormalReceiptErrorV4("COMMAND_RESULT_STATUS_INVALID")
    return value


def build_verification_receipt_v4(
    *, repo_root: Path, command_result_path: Path, output_path: Path
) -> dict[str, Any]:
    require_external_output_v4(repo_root, output_path)
    source = validate_command_result_v4(command_result_path)
    kind = source["receipt_kind"]
    if kind not in VERIFICATION_KINDS_V4:
        raise P5FormalReceiptErrorV4("VERIFICATION_KIND_INVALID")
    ...
    if kind == "p4":
        ...
        if p4_path is None:
            raise P5FormalReceiptErrorV4("P4_GATE_MANIFEST_BINDING_MISSING")
        p4 = _load_json(p4_path, "P4_GATE_MANIFEST_INVALID")
        solver = p4.get("solver_admission")
        if not isinstance(solver, dict):
            raise P5FormalReceiptErrorV4("P4_SOLVER_ADMISSION_MISSING")
    ...
    return validate_verification_receipt_v4(output_path)


def validate_verification_receipt_v4(path: Path) -> dict[str, Any]:
    ...
    if (
        set(value) != expected
        or value.get("schema_version") != "trip-check-p5-verification-receipt-v4"
        or value.get("receipt_kind") not in VERIFICATION_KINDS_V4
        or value.get("status") not in RECEIPT_STATUSES_V4
        or value.get("upstream_commit") != value.get("subject_commit")
        or value.get("dirty_tree") is not False
        ...
        raise P5FormalReceiptErrorV4("VERIFICATION_RECEIPT_CONTRACT_INVALID")
    command_path = Path(command["path"])
    source = validate_command_result_v4(command_path)
    readback = (
    ...
    if value["status"] == "PASS" and (
        ...
        raise P5FormalReceiptErrorV4("VERIFICATION_PASS_NOT_PROVEN")
    if value["receipt_kind"] == "p4" and value.get("solver_admission") != {
        "status": "REJECT",
        "default_strategy": "bounded_repair_v1",
    }:
        raise P5FormalReceiptErrorV4("P4_SOLVER_ADMISSION_INVALID")
    return value


def build_dataset_formal_validation_receipt_v4(
    *,
    repo_root: Path,
    ...
    if source["receipt_kind"] != "dataset_formal":
        raise P5FormalReceiptErrorV4("DATASET_RECEIPT_COMMAND_KIND_INVALID")
    stdout_records = [
        item
        for item in source["artifact_bindings"]
    ...
    if len(stdout_records) != 1:
        raise P5FormalReceiptErrorV4("DATASET_VALIDATION_STDOUT_MISSING")
    result = _load_json(
    ...
    if not isinstance(files, dict):
        raise P5FormalReceiptErrorV4("DATASET_MANIFEST_FILE_INDEX_INVALID")
    ...
    for name, entry in sorted(files.items()):
        ...
        if dataset_files[name]["sha256"] != entry.get("file_sha256"):
            raise P5FormalReceiptErrorV4("DATASET_FILE_HASH_MISMATCH")
    errors: list[str] = []
    if source["status"] != "PASS":
        errors.append("DATASET_VALIDATOR_COMMAND_NOT_PASS")
    if (
        result.get("schema_version") != "trip-check-p5-dataset-validation-v4"
    ...
    receipt: dict[str, Any] = {
        "schema_version": "trip-check-p5-dataset-formal-validation-receipt-v4",
    ...
    receipt["receipt_hash"] = digest(receipt)
    _atomic_json(output_path, receipt)
    return validate_dataset_formal_validation_receipt_v4(output_path)


def validate_dataset_formal_validation_receipt_v4(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        != "trip-check-p5-dataset-formal-validation-receipt-v4"
        ...
        raise P5FormalReceiptErrorV4("DATASET_FORMAL_RECEIPT_CONTRACT_INVALID")
    ...
    readback = (
        ...
        and result.get("schema_version") == "trip-check-p5-dataset-validation-v4"
    ...
    if value["status"] == "PASS" and (
        ...
        raise P5FormalReceiptErrorV4("DATASET_FORMAL_PASS_NOT_PROVEN")
    return value


def build_formal_gate_receipt_v4(
    *,
    repo_root: Path,
    ...
    require_external_output_v4(repo_root, output_path)
    if set(verification_receipts) != set(VERIFICATION_KINDS_V4) or set(
        primary_artifacts
    ) != set(PRIMARY_ARTIFACT_NAMES_V4):
        raise P5FormalReceiptErrorV4("FORMAL_RECEIPT_INPUT_SET_INVALID")
    binding = repo_binding or read_repo_binding_v4(repo_root)
    _require_formal_repo_binding(binding)
    dataset_receipt = validate_dataset_formal_validation_receipt_v4(
        dataset_receipt_path
    )
    wrappers = {
        kind: validate_verification_receipt_v4(path)
    ...
    receipt["receipt_hash"] = digest(receipt)
    _atomic_json(output_path, receipt)
    validate_formal_gate_receipt_v4(output_path)
    return receipt


def validate_formal_gate_receipt_v4(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or set(bindings)
        != {f"{name}_sha256" for name in PRIMARY_ARTIFACT_NAMES_V4}
        ...
        or set(verification_entries) != set(VERIFICATION_KINDS_V4)
        or any(
        ...
        raise P5FormalReceiptErrorV4("FORMAL_GATE_RECEIPT_CONTRACT_INVALID")
    dataset_path = Path(dataset_entry["path"])
    dataset_receipt = validate_dataset_formal_validation_receipt_v4(dataset_path)
    wrappers = {
        kind: validate_verification_receipt_v4(Path(entry["path"]))
    ...
    if value["status"] == "PASS" and (
        ...
        raise P5FormalReceiptErrorV4("FORMAL_GATE_PASS_NOT_PROVEN")
    return value


def mint_blind_nonce_v4(
    *,
    repo_root: Path,
    output_path: Path,
    ...
    destination = require_external_output_v4(repo_root, output_path)
    if destination.exists():
        raise P5FormalReceiptErrorV4("BLIND_NONCE_OVERWRITE_FORBIDDEN")
    binding = repo_binding or read_repo_binding_v4(repo_root)
    _require_formal_repo_binding(binding)
    ...
    if (
        active.get("active_contract") != ACTIVE_CONTRACT_V4
        or active.get("formal_evidence_status") != "READY"
        or dataset.get("dataset_id") != DATASET_ID_V4
        ...
        or active.get("blind_seal_v4_sha256") != _sha256(seal_path)
    ):
        raise P5FormalReceiptErrorV4("BLIND_NONCE_FORMAL_DATASET_NOT_READY")
    nonce = {
        "schema_version": "trip-check-p5-blind-run-nonce-v4",
        "purpose": "execute_frozen_blind_once",
        "dataset_id": DATASET_ID_V4,
        "active_contract": ACTIVE_CONTRACT_V4,
    ...
    if errors:
        raise P5FormalReceiptErrorV4("BLIND_NONCE_SCHEMA_REJECTED")
    _atomic_json(destination, nonce)
    readback = _load_json(destination, "BLIND_NONCE_READBACK_FAILED")
    if readback != nonce:
        raise P5FormalReceiptErrorV4("BLIND_NONCE_READBACK_FAILED")
    serialized = json.dumps(readback, sort_keys=True).lower()
    if any(token in serialized for token in ("label", "oracle", "answer", "expected")):
        raise P5FormalReceiptErrorV4("BLIND_NONCE_FORBIDDEN_PAYLOAD")
    return {
        "schema_version": "trip-check-p5-blind-run-nonce-mint-receipt-v4",
