"""Formal deterministic scorer for a validated P5 v5 non-blind run group."""
...
from evals.trip_check_v1.p5.nonblind_runner_v5 import (
...
SCORE_REPORT_SCHEMA_V5 = "trip-check-p5-nonblind-score-report-v5"
CASE_SCORE_SCHEMA_V5 = "trip-check-p5-case-score-v5"


class P5NonblindScoringErrorV5(RuntimeError):
    """Stable fail-closed scorer error for invalid v5 evidence."""
...
class P5CaseScoreV5(_scorer_v3.P5CaseScoreV3):
    """Versioned case score; semantics are deliberately inherited from v3."""

    model_config = ConfigDict(frozen=True, extra="forbid")
    schema_version: Literal["trip-check-p5-case-score-v5"] = CASE_SCORE_SCHEMA_V5


class P5NonblindScoreReportV5(BaseModel):
    """Independent v5 report envelope over deterministic v3 score semantics."""

    model_config = ConfigDict(frozen=True, extra="forbid")

    schema_version: Literal["trip-check-p5-nonblind-score-report-v5"] = (
        SCORE_REPORT_SCHEMA_V5
    )
    ...
    dataset_id: Literal["trip-check-p5-360-v5"] = DATASET_ID_V5
    ...
    case_count: Literal[270] = NONBLIND_CASE_COUNT_V5
    terminal_count: Literal[810] = NONBLIND_TERMINAL_COUNT_V5
    replay_readback_count: Literal[810] = NONBLIND_TERMINAL_COUNT_V5
    variant_metrics: dict[str, Any]
    paired_comparisons: dict[str, Any]
    zero_tolerance_checks: dict[str, bool]
    stage_gate_checks: dict[str, bool]
    promotion_decision: Literal["KEEP_CORE_B", "REJECT_ALL_CANDIDATES"]
    ...
    case_scores: list[P5CaseScoreV5]
    report_hash: str = Field(pattern=r"^[0-9a-f]{64}$")

    @model_validator(mode="after")
    def hash_and_decision_are_bound(self) -> "P5NonblindScoreReportV5":
        payload = self.model_dump(mode="json", exclude={"report_hash"})
        if self.report_hash != digest(payload):
            raise ValueError("report_hash does not bind the complete v5 report")
        ...
        if (
            self.status == "REJECT"
            and self.promotion_decision != "REJECT_ALL_CANDIDATES"
        ):
            raise ValueError("REJECT must reject every candidate")
        if set(self.variant_metrics) != set(VARIANT_IDS_V5):
            raise ValueError("variant metrics do not bind the exact v5 variant set")
        if len(self.case_scores) != NONBLIND_TERMINAL_COUNT_V5:
            raise ValueError("case scores do not bind all 810 terminals")
        return self


def _project_terminal_to_v3(output: P5TerminalOutputV5) -> P5TerminalOutputV3:
    """Rebind a validated v5 terminal into scorer_v3's semantic hash domain."""
...
def _require_validated_shape(
    *,
    ...
    outputs: Sequence[P5TerminalOutputV5],
    ...
    if (
        manifest.get("schema_version") != "trip-check-p5-run-group-v5"
        or manifest.get("lane") != "nonblind"
        or manifest.get("status") != "PASS"
        or manifest.get("dataset_id") != DATASET_ID_V5
        or manifest.get("case_count") != NONBLIND_CASE_COUNT_V5
        or manifest.get("terminal_count") != NONBLIND_TERMINAL_COUNT_V5
        or manifest.get("replay_readback_count") != NONBLIND_TERMINAL_COUNT_V5
        or manifest.get("variant_ids") != list(VARIANT_IDS_V5)
        ...
        or len(cases) != NONBLIND_CASE_COUNT_V5
        or len(case_ids) != NONBLIND_CASE_COUNT_V5
        or len(outputs) != NONBLIND_TERMINAL_COUNT_V5
        or len(actual_keys) != len(set(actual_keys))
        or set(actual_keys) != expected_keys
        or set(materializations) != case_ids
    ):
        raise P5NonblindScoringErrorV5("V5_NONBLIND_VALIDATED_SHAPE_INVALID")
    if formal_validation_performed and manifest.get("formal_evidence") is not True:
        raise P5NonblindScoringErrorV5("V5_NONBLIND_FORMAL_RUN_REQUIRED")


def build_nonblind_score_report_v5(
    *,
    manifest: Mapping[str, Any],
    cases: Sequence[P5CaseV3],
    outputs: Sequence[P5TerminalOutputV5],
    ...
    """Reuse v3 case/aggregate semantics and emit a hash-bound v5 envelope."""
    ...
    try:
        ...
        case_scores = [
            P5CaseScoreV5.model_validate(
                {**item, "schema_version": CASE_SCORE_SCHEMA_V5}
            ).model_dump(mode="json")
            for item in raw_case_scores
        ]
    except P5NonblindScoringErrorV5:
        raise
    except Exception as exc:
        reason = getattr(exc, "reason_code", "V5_NONBLIND_SCORING_FAILED")
        raise P5NonblindScoringErrorV5(str(reason)) from exc
    ...
    payload: dict[str, Any] = {
        "schema_version": SCORE_REPORT_SCHEMA_V5,
        ...
        "scoring_semantics": {
            ...
            "terminal_projection": "v5-to-v3-semantic-hash-rebinding",
            "source_terminal_schema": "trip-check-p5-terminal-output-v5",
            "report_envelope": SCORE_REPORT_SCHEMA_V5,
        },
        ...
        "promotion_decision": (
            "KEEP_CORE_B" if passed else "REJECT_ALL_CANDIDATES"
        ),
    ...
    try:
        report = P5NonblindScoreReportV5.model_validate(payload)
    except Exception as exc:
        raise P5NonblindScoringErrorV5("V5_NONBLIND_REPORT_INVALID") from exc
    return report.model_dump(mode="json")


def score_nonblind_run_group_v5(
    *,
    run_dir: Path,
    repo_root: Path,
    ...
    """Validate the v5 run first; diagnostic scoring can never return PASS."""

    validator = run_validator or validate_nonblind_run_group_v5
    try:
        ...
        return build_nonblind_score_report_v5(
            manifest=manifest,
            cases=cases,
            outputs=outputs,
            materializations=materializations,
        ...
        raise
    except (P5NonblindRunnerErrorV5, _scorer_v3.P5V3ScoringError) as exc:
        raise P5NonblindScoringErrorV5(exc.reason_code) from exc
    except Exception as exc:
        raise P5NonblindScoringErrorV5("V5_NONBLIND_SCORING_FAILED") from exc


__all__ = [
    "CASE_SCORE_SCHEMA_V5",
    "P5CaseScoreV5",
    "P5NonblindScoreReportV5",
