...
async def _cancellation_contract_holds() -> bool:
    ...
    for verb in ("取消", "后来不去"):
        distinct_name = await _run_text_pipeline(
            f"杭州一日游。Day 1 去西湖；{verb}西湖风景名胜区。"
        )
        distinct_name_cards = [
            card.name
            for day in distinct_name.public_result.days
            for card in day.activities
        ]
        distinct_name_activities = [
            (
                item.compiled.mention.atomic_place_name,
                item.compiled.mention.role,
                item.compiled.eligible_for_place_search,
            )
            for item in distinct_name.activities
        ]
        if distinct_name_cards != ["西湖"] or distinct_name_activities != [
            ("西湖", ActivityRole.PLANNED, True),
            ("西湖风景名胜区", ActivityRole.EXCLUDED, False),
        ] or distinct_name.inference_binding.get(
            "terminal_cancellation_reclassification_count", 0
        ):
            return False
