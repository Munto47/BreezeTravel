"""Custodian-only CLI for aggregate scoring of one P5 v5 blind run."""
...
from evals.trip_check_v1.p5.final_blind_scorer_v5 import (  # noqa: E402
...
def _write_external(path: Path, value: dict) -> None:
    ...
    try:
        ...
        raise P5BlindScoringErrorV5("BLIND_SCORE_OUTPUT_INSIDE_REPOSITORY")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        reader = ExternalCustodianBundleReaderV5.from_external_files(
            bundle_path=args.bundle,
            authorization_path=args.custodian_authorization,
            repo_root=REPO_ROOT,
        )
        receipt = score_isolated_blind_v5(
            repo_root=REPO_ROOT,
    ...
    except P5BlindScoringErrorV5 as exc:
        print(
            json.dumps(
                {
