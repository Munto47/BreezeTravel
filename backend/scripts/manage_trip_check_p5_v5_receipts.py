"""Run, wrap, aggregate, and validate real P5 v5 formal receipts."""
...
from evals.trip_check_v1.p5.formal_receipts_v5 import (  # noqa: E402
    COMMAND_KINDS_V5,
...
def _primary(args: argparse.Namespace) -> dict[str, Path]:
    paths = {
        ...
        "judge_calibration_panel": args.judge_calibration_panel,
...
def parser() -> argparse.ArgumentParser:
    ...
    run.add_argument("--kind", choices=COMMAND_KINDS_V5, required=True)
    ...
    dataset.add_argument(
        "--validator",
        type=Path,
        default=REPO_ROOT / "backend" / "scripts" / "validate_trip_check_p5_dataset_v5.py",
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.action == "run":
        ...
        if command[:1] == ["--"]:
            command = command[1:]
        receipt = execute_command_receipt_v5(
            repo_root=REPO_ROOT,
            kind=args.kind,
            command=command,
        ...
        receipt = build_dataset_formal_validation_receipt_v5(
            repo_root=REPO_ROOT,
        ...
        receipt = build_verification_receipt_v5(
            repo_root=REPO_ROOT,
            command_result_path=args.command_result,
            output_path=args.output,
        )
        ...
        if set(verifications) != set(VERIFICATION_KINDS_V5):
            raise SystemExit("--verification must bind exactly the eight required kinds")
        receipt = build_formal_gate_receipt_v5(
            repo_root=REPO_ROOT,
        ...
        validators = {
            "command": validate_command_result_v5,
            "dataset": validate_dataset_formal_validation_receipt_v5,
            "verification": validate_verification_receipt_v5,
            "formal": validate_formal_gate_receipt_v5,
        }
