"""Execute an exact P5 v5 non-blind run outside the repository."""
...
from evals.trip_check_v1.p5.nonblind_runner_v5 import (  # noqa: E402
    run_nonblind_v5,
)
...
async def execute(args: argparse.Namespace) -> dict[str, object]:
    ...
    run_id = args.run_id or datetime.now(timezone.utc).strftime(
        "p5-v5-nonblind-%Y%m%dT%H%M%SZ"
    )
    return await run_nonblind_v5(
        repo_root=REPO_ROOT,
