"""Export three slot-bound P5 Judge v3 sealed-holdout inputs."""
...
def main() -> None:
    ...
    parser.add_argument("--protocol", type=Path, default=P5_ROOT / "judge_protocol_v3.json")
    ...
    output = args.custody_output_dir / "judge_holdout_export_receipt.v3.json"
    payload = json.dumps(receipt, ensure_ascii=False, indent=2, sort_keys=True) + "\n"
    output.write_text(payload, encoding="utf-8", newline="\n")
    if json.loads(output.read_text(encoding="utf-8")) != receipt:
        raise RuntimeError("P5_JUDGE_V3_HOLDOUT_EXPORT_READBACK_FAILED")
    print(payload, end="")
