"""Build the label-free P5 v3 semantic-rebinding dataset."""
...
def run(*, write: bool) -> dict:
    nonblind, blind, nonblind_materializations, blind_materializations = build_dataset_v3()
    result = {
        "schema_version": "trip-check-p5-build-result-v3",
        "status": "PASS",
        "write": write,
        "source_mode": "SEALED_V2_SOURCE_REBIND",
        ...
        "counts": {
            ...
            "historical_ocr_receipts": sum(
                row["ocr_baseline_receipt"] is not None
                for row in [*nonblind_materializations, *blind_materializations]
            ),
        },
    }
    if write:
        manifest = write_dataset_v3(
