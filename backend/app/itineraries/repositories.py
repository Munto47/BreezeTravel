...
def _workspace_from_row(row: Any) -> TripWorkspace:
    return TripWorkspace(
        ...
        current_brief_id=row.get("current_brief_id"),
        current_trip_brief_revision=row.get("current_trip_brief_revision"),
        current_trip_check_run_id=row.get("current_trip_check_run_id"),
        status=row["status"],
