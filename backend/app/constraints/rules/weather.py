...
class WeatherRule:
    ...
    def evaluate(self, context: RuleContext) -> list[ConstraintCheck]:
        ...
        for day in context.itinerary.days:
            ...
            checks.append(ConstraintCheck(
                constraint_id=constraint.id,
                status=ConstraintStatus.VIOLATED if violated else ConstraintStatus.SATISFIED,
                reason_code="RAIN_OUTDOOR_CONFLICT" if violated else "WEATHER_CONSTRAINT_OK",
                message=f"第 {day.day_index + 1} 天" + (f"雨天仍有 {len(outdoor)} 个户外地点" if violated else "天气安排符合约束"),
                day_index=day.day_index,
