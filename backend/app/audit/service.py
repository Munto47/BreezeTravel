...
class AuditApplicationService:
    ...
    async def _prepare_current_audit(
        ...
        *,
        ...
        observations = evidence_observations if evidence_observations is not None else self.evidence_service.observations_from_revision(
            revision,
            place_records,
            now=now,
            target_itinerary_revision=revision.revision,
        ...
        resolution_versions = {stop.place_id: 1 for day in revision.days for stop in day.stops}
        ...
        report = self.engine.run(
            run_input=AuditRunInput(
                ...
                place_resolution_versions=resolution_versions,
            ),
            revision=revision,
            task_spec=task_spec,
        ...
        return workspace, snapshot, report, self._workspace_basis(workspace)
