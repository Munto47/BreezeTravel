...
def _dataset(tmp_path: Path, *, subject_commit: str) -> BlindDatasetPathsV4:
    data = tmp_path / "data"
    data.mkdir()
    inputs = [
        {
    ...
    seal = {
        ...
        "variant_ids_sha256": digest(list(VARIANT_IDS_V4)),
    ...
    active = {
        ...
        "blind_seal_v4_sha256": file_sha256(seal_path),
        "candidate_freeze_commit": subject_commit,
    }
...
class _Engine:
    def __init__(self, *, ocr_lookup_count: int = BLIND_OCR_REPLAY_COUNT_V4) -> None:
        self.ocr_lookup_count = ocr_lookup_count

    async def execute(self, **kwargs) -> BlindExecutionResultV4:
        ...
        for variant_id in VARIANT_IDS_V4:
            for case in case_rows:
                ...
                terminal["semantic_output_hash"] = semantic_output_hash_v4(terminal)
        ...
        return BlindExecutionResultV4(
            ...
            run_specs={variant_id: {"variant_id": variant_id} for variant_id in VARIANT_IDS_V4},
...
def _run(tmp_path: Path, *, engine: _Engine, run_id: str = "blind-v4-test") -> dict:
    subject = "1" * 40
    ...
    return asyncio.run(
        run_blind_once_v4(
            ...
            upstream_ref="origin/codex/p5-v4-blind-score",
...
def test_v4_blind_runner_writes_exact_external_shape_and_consumes_nonce(tmp_path: Path) -> None:
...
def test_v4_blind_runner_rejects_reusing_nonce_even_with_new_run_id(tmp_path: Path) -> None:
    ...
    del result
    with pytest.raises(P5BlindRunnerErrorV4, match="BLIND_NONCE_ALREADY_CONSUMED"):
        asyncio.run(
            run_blind_once_v4(
                repo_root=Path(__file__).resolve().parents[2],
                dataset_paths=paths,
...
def test_v4_ocr_count_drift_fails_and_nonce_remains_consumed(tmp_path: Path) -> None:
    with pytest.raises(P5BlindRunnerErrorV4, match="BLIND_OCR_REPLAY_PROVENANCE_INVALID"):
