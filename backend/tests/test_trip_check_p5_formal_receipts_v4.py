...
def test_real_command_result_can_be_wrapped_and_tamper_fails_closed(
    ...
    wrapper = build_verification_receipt_v4(
        repo_root=repo,
    ...
    with pytest.raises(P5FormalReceiptErrorV4):
        validate_verification_receipt_v4(wrapper_path)
...
def test_blind_nonce_is_external_single_use_shaped_and_label_free(
    ...
    seal = p5 / "sealed" / "frozen_blind.v4.seal.json"
    dataset = p5 / "dataset_v4.manifest.json"
    active = p5 / "active_contract.json"
    _write_json(seal, {"schema_version": "trip-check-p5-blind-seal-v4"})
    manifest = {
    ...
    schema = (
        ...
        / "trip_check_v1"
        / "p5"
        / "blind_run_nonce_v4.schema.json"
    )
    ...
    with pytest.raises(P5FormalReceiptErrorV4, match="OVERWRITE_FORBIDDEN"):
        mint_blind_nonce_v4(
            repo_root=repo,
            output_path=output,
            active_contract_path=active,
    ...
    with pytest.raises(P5FormalReceiptErrorV4, match="OUTPUT_MUST_BE_EXTERNAL"):
        mint_blind_nonce_v4(
            repo_root=repo,
