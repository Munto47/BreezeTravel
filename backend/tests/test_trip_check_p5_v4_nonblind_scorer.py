...
from evals.trip_check_v1.p5 import nonblind_scorer_v4 as scorer
from evals.trip_check_v1.p5.data_contract import digest
from evals.trip_check_v1.p5.gate_v4 import parse_nonblind_score_v4
...
def _terminal_v4() -> P5TerminalOutputV4:
    ...
    return P5TerminalOutputV4.model_validate(payload)
...
def _manifest(*, formal: bool) -> dict[str, Any]:
    payload = {
        ...
        "upstream_ref": "origin/codex/p5-v4",
...
def _case_score(index: int) -> dict[str, Any]:
    variant_id = VARIANT_IDS_V4[index % len(VARIANT_IDS_V4)]
    return {
...
def test_terminal_projection_rebinds_to_v3_semantic_hash_domain() -> None:
    ...
    projected = scorer._project_terminal_to_v3(terminal_v4)
    ...
    assert projected.semantic_output_hash != terminal_v4.semantic_output_hash


def test_formal_report_is_v4_hash_bound_and_gate_compatible(monkeypatch) -> None:
    ...
    report = scorer.build_nonblind_score_report_v4(
        manifest=manifest,
        cases=cases,
        outputs=outputs,
    ...
    assert {item["schema_version"] for item in report["case_scores"]} == {
        "trip-check-p5-case-score-v4"
    }
    assert report["report_hash"] == digest(
        {key: value for key, value in report.items() if key != "report_hash"}
    )
    assert parse_nonblind_score_v4(report, run=manifest)["status"] == "PASS"
...
def test_development_report_always_rejects_even_when_metrics_pass(monkeypatch) -> None:
    ...
    assert report["promotion_decision"] == "REJECT_ALL"
...
def test_score_entrypoint_calls_runner_validator_before_build(monkeypatch) -> None:
    ...
    report = scorer.score_nonblind_run_group_v4(
