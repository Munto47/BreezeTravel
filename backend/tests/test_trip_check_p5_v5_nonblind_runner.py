...
from evals.trip_check_v1.p5 import nonblind_runner_v5
...
class _FixtureEngine:
    def __init__(self, *, lookup_count: int = NONBLIND_OCR_LOOKUP_COUNT_V5) -> None:
...
def test_nonblind_runner_writes_exact_v5_manifest_and_readback(tmp_path: Path) -> None:
    subject = "a" * 40
    result = asyncio.run(
        run_nonblind_v5(
            repo_root=REPO_ROOT,
            output_root=tmp_path,
            run_id="fixture-v5-nonblind",
    ...
    assert result["schema_version"] == "trip-check-p5-run-group-v5"
    assert result["case_count"] == NONBLIND_CASE_COUNT_V5
    assert result["terminal_count"] == NONBLIND_TERMINAL_COUNT_V5
    assert result["replay_readback_count"] == NONBLIND_TERMINAL_COUNT_V5
    ...
    manifest, cases, outputs, materializations = validate_nonblind_run_group_v5(
...
def test_formal_runner_fails_before_execution_until_v5_seal_is_active(
    tmp_path: Path,
) -> None:
    engine = _FixtureEngine()
    subject = "c" * 40
    unsealed_paths = replace(
        nonblind_runner_v5._default_dataset_paths(),
        seal=tmp_path / "missing-v5-seal.json",
    )
    with pytest.raises(
        P5NonblindRunnerErrorV5,
        ...
        asyncio.run(
            run_nonblind_v5(
                repo_root=REPO_ROOT,
                output_root=tmp_path,
                run_id="fixture-v5-formal-not-ready",
                subject_commit=subject,
                upstream_ref="origin/fixture",
                upstream_commit=subject,
                dirty_tree=False,
