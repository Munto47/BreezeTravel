...
from evals.trip_check_v1.p5.final_blind_scorer_v5 import (
    P5BlindScoringErrorV5,
    _default_case_scorer,
    canonical_labels_hash_v5,
    score_isolated_blind_v5,
)
from evals.trip_check_v1.p5.runner_v5 import BlindDatasetPathsV5, VARIANT_IDS_V5
...
) -> tuple[dict, _Reader, BlindDatasetPathsV5, callable]:
    cases = [
        {
        ...
        json.dumps(
            {
                ...
                "labels_canonical_sha256": canonical_labels_hash_v5(labels),
            }
        ),
        encoding="utf-8",
    )
...
def test_v5_scorer_returns_only_variant_aggregates_without_label_side_channels(
    tmp_path: Path,
) -> None:
    manifest, reader, paths, validator = _fixture(tmp_path)
    report = score_isolated_blind_v5(
    ...
    assert set(report["variant_metrics"]) == set(VARIANT_IDS_V5)
...
def test_v5_scorer_rejects_non_custodian_before_bundle_read(tmp_path: Path) -> None:
    _manifest, reader, paths, validator = _fixture(tmp_path)
    reader.role = "developer"  # type: ignore[misc]

    with pytest.raises(P5BlindScoringErrorV5, match="CUSTODIAN_AUTHORIZATION_BINDING_MISMATCH"):
        score_isolated_blind_v5(
            repo_root=tmp_path / "repo",
            run_dir=tmp_path / "run",
            expected_bundle_sha256=reader.bundle_sha256,
...
def test_v5_scorer_accepts_each_compatible_oracle_payload_mode(
    tmp_path: Path,
    candidate_mode: str,
) -> None:
...
def test_v5_scorer_rejects_semantic_mismatch_before_case_scorer_without_disclosure(
    tmp_path: Path,
    candidate_mode: str,
    ...
    with pytest.raises(P5BlindScoringErrorV5) as caught:
        score_isolated_blind_v5(
            repo_root=tmp_path / "repo",
            run_dir=tmp_path / "run",
            expected_bundle_sha256=reader.bundle_sha256,
...
def test_v5_cli_emits_only_invalid_evidence_for_semantic_mismatch(
    monkeypatch: pytest.MonkeyPatch,
    capsys: pytest.CaptureFixture[str],
    tmp_path: Path,
) -> None:
    from scripts import score_trip_check_p5_v5_blind as cli

    monkeypatch.setattr(
        cli.ExternalCustodianBundleReaderV5,
        "from_external_files",
    ...
    def reject_semantics(**kwargs):
        del kwargs
        raise P5BlindScoringErrorV5("BLIND_ORACLE_PAYLOAD_SEMANTIC_MISMATCH")

    monkeypatch.setattr(cli, "score_isolated_blind_v5", reject_semantics)
    ...
    assert json.loads(captured.err) == {
        "schema_version": "trip-check-p5-isolated-blind-score-error-v5",
...
def test_v5_default_case_scorer_reuses_v3_deterministic_semantics(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    from evals.trip_check_v1.p5 import nonblind_scorer_v5, scorer_v3
