...
from evals.trip_check_v1.p5.judge_v4 import (
    P5JudgeErrorV4,
...
def _round_report(round_index: int, bundle_receipt: dict[str, object]) -> dict:
    ...
    return {
        "schema_version": "trip-check-p5-judge-round-v4",
...
def test_v4_export_is_anonymous_and_aggregates_three_independent_rounds(
    tmp_path: Path,
) -> None:
    ...
    for round_index, round_dir in enumerate(round_dirs, 1):
        bundle_path = round_dir / f"judge_input_round_{round_index}.v4.json"
    ...
    mapping_path = tmp_path / "custody" / "judge_variant_mapping.v4.json"
    round_paths = _round_paths(tmp_path, receipt, round_dirs)
    panel = aggregate_judge_rounds_v4(
        repo_root=repo_root,
...
def test_v4_aggregation_rejects_shared_round_context(tmp_path: Path) -> None:
    repo_root, receipt, round_dirs = _export(tmp_path)
    mapping_path = tmp_path / "custody" / "judge_variant_mapping.v4.json"
    round_paths = _round_paths(tmp_path, receipt, round_dirs)
    ...
    with pytest.raises(P5JudgeErrorV4, match="JUDGE_ROUND_INDEPENDENCE_INVALID"):
        aggregate_judge_rounds_v4(
            repo_root=repo_root,
            mapping_path=mapping_path,
            mapping_sha256=hashlib.sha256(mapping_path.read_bytes()).hexdigest(),
...
def test_v4_aggregation_rejects_any_observed_peer_result(tmp_path: Path) -> None:
    repo_root, receipt, round_dirs = _export(tmp_path)
    mapping_path = tmp_path / "custody" / "judge_variant_mapping.v4.json"
    round_paths = _round_paths(tmp_path, receipt, round_dirs)
    ...
    with pytest.raises(P5JudgeErrorV4, match="JUDGE_ROUND_CONTRACT_INVALID"):
        aggregate_judge_rounds_v4(
            repo_root=repo_root,
            mapping_path=mapping_path,
            mapping_sha256=hashlib.sha256(mapping_path.read_bytes()).hexdigest(),
